...
def _getRestOfURL(splitURL):
    ...
    """
    print('[DEBUG] _getRestOfURL :: splitURL:', tuple(splitURL))
    parts = ("", "") + tuple(splitURL)[2:]
    print(parts)
    return tuple(urlparse(parts[2])) or b"/"
...
class MinitrueRequest(proxy.ProxyRequest):
    ...
    def process(self):
        ...
        print("Request from %s for %s" % (
            self.getClientAddress(), self.getAllHeaders().get(b'host')))
    ...
    def _finishProcessing(self):
        ...
        print('[DEBUG] MinitrueRequest(proxy.ProxyRequest) -> _finishProcessing :: url:', url)
        host, port = self._getHostAndPort(url.netloc, url.scheme)
        rest = _getRestOfURL(url)
        rest = rest[2] if isinstance(rest, tuple) else rest
        print('[DEBUG] MinitrueRequest(proxy.ProxyRequest) -> _finishProcessing :: rest:', rest)
        headers = self._buildHeaders(host)

        builder = self._getClientFactoryBuilder(url.scheme.decode())
        clientFactory = builder(path=rest, headers=headers)
        self.reactor.connectTCP(host, port, clientFactory)

    def _getClientFactoryBuilder(self, scheme):
        scheme = scheme or "http"
        print('[DEBUG] MinitrueRequest(proxy.ProxyRequest) -> _getClientFactoryBuilder :: self.protocols:', self.protocols)
    ...
    def _getHostAndPort(self, netloc, scheme):
        ...
        """
        scheme = scheme or b"http"
        host, port = netloc, self.ports[scheme]
        print('[DEBUG] MinitrueRequest(proxy.ProxyRequest) -> _getHostAndPort :: host:', host)
        if b":" in host:
    ...
    def _buildHeaders(self, host):
        ...
        hasn't been set.
        """
        print('[DEBUG] MinitrueRequest(proxy.ProxyRequest) -> _buildheaders :: host:', host)
...
reactor.listenTCP(8080, MinitrueFactory())
reactor.run()